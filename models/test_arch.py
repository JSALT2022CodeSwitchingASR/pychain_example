...
# testing tdnn + fnet outputs concatenation
...
class TEST_ARCH(nn.Module):
    def __init__(self, in_dim, out_dim, tdnn_num_layers, tdnn_out_dim, fnet_num_layers, hidden_dims, kernel_sizes, strides, dilations, dropout, residual=False, fnet_activation=nn.GELU):
        ...
        self.fnet_layers = nn.ModuleList([FNetEncoderBlock(hidden_dims[tdnn_num_layers -1], hidden_dims[i], dropout, fnet_activation()) for i in range(tdnn_num_layers, len(hidden_dims))])
        self.linear = nn.Linear(2*hidden_dims[tdnn_num_layers -1], out_dim)
    ...
    def forward(self, x, x_lengths):
        ...
        for layer in (self.fnet_layers):
            x, x_lengths = layer(x, x_lengths)	
        # concatenate outputs of TDNN blocks and FNET blocks
        x = self.linear(torch.cat([x, h], dim=2))
        return x, x_lengths
